...
var
  Form5: TForm5;
   name2,ocenka:string;
   assessment:real;
 kolotv,numvopr,i,otv,sbrosotv:Integer;                      //otv - ответы
 numvcl:array[1..15] of Integer;
 numotv:array[1..12,1..7] of Integer;             //номера ответов!!!
 shap:array[1..7] of Integer;
 butend,f4:integer;
implementation
uses Unit1, Unit2;
{$R *.dfm}
...
case numvopr of
 ...
 kolotv:=kolotv+1;                    //  numotv[1(номера вопроса),1(номер ответа)]:=1;
...
case numvopr of            // выключает выбор ответа если ответили    (номер вопроса)
1: numvcl[1]:=1;
2: numvcl[2]:=1;
3: numvcl[3]:=1;
4: numvcl[4]:=1;
...
case numvopr of                            //показывает вариант ответа
...
procedure TForm5.Button3Click(Sender: TObject);      //показать выбранный вариант ответа
...
for  i:=1 to 12 do                                            //отключение выбора ответа после окончания теста
 numvcl[i]:=1;
 otv:=1;
...
next(-numvopr);
 ...
 //ShowMessage('Количество ответов = '+IntToStr(kolotv));
 rezult := MessageDlg('Оценка = '+ocenka+#13#10+'Сохранить результат?',mtInformation, [mbYes, mbNo], 0);

  if rezult = mrYes then begin
   a:='доп материал\test.txt';                       //-------------------------сохранение
   AssignFile(f2,'доп материал\test.txt');
    if FileExists(a) then
     Append(f2)
     else if FileExists(a)=false then
      Rewrite(f2);
  { if rezult = mrYes then                       //-------------------------сохранение
  if  Saveotv.Execute then begin
   ...
   WriteLn(f2,'number2');
   for b := 1 to 5 do begin                                  //ввод ответов в блокнот
    for c := 1 to 7 do
     Write(f2,numotv[b,c]);
      WriteLn(f2);
   end;

      for b := 6 to 12 do begin                                   //ввод ответов в блокнот
    for c := 1 to 7 do
     Write(f2,numotv[b,c]);
     WriteLn(f2);
      end;
...
a:String;
begin
  if (butend=0) and (otv=0)and  (f4<>1) then  begin
...
procedure TForm5.next(a: Integer);           // + сделано (если ответили то если вернуться на этот вопрос то он не активен и сохронится вариант ответа)
begin
 numvopr:=numvopr+a;
   shapclear;                       //+доделать  вариант ответа   --  двумерный массив
 case numvopr of
   1: numb1;
   2: numb2;
   3: numb3;
...
Label1.Caption:='После выключения компьютера информация стирается из ...';
Rb1.Caption:='HDD';
Rb2.Caption:='ОЗУ';
Rb3.Caption:='процессор';

Rab1.Caption:='HDD';
Rab2.Caption:='ОЗУ';
Rab3.Caption:='процессор';
Rab2.Checked:=true;
...
Label1.Caption:='Совокупность программ, позволяющая организовать решение задач на ЭВМ.';
Rb1.Caption:='аппаратное обеспечение';
Rb2.Caption:='программное обеспечение';
Rb3.Caption:='компилятор';

Rab1.Caption:='аппаратное обеспечение';
Rab2.Caption:='программное обеспечение';
Rab3.Caption:='компилятор';
Rab2.Checked:=true;
...
GroupBox3.Visible:=false;
 ...
 Label1.Caption:='Расшифровка BIOS';
Rb1.Caption:='базовая система программирования';
Rb2.Caption:='базовая информация о системе';
Rb3.Caption:='базовая система ввода-вывода';

Rab1.Caption:='базовая система программирования';
Rab2.Caption:='базовая информация о системе';
Rab3.Caption:='базовая система ввода-вывода';
Rab3.Checked:=true;
...
GroupBox3.Visible:=false;
  ...
  Label1.Caption:='Аппаратное подключение периферийного устройства к магистрали производится через ...';
Rb1.Caption:='контроллер';
Rb2.Caption:='регистр';
Rb3.Caption:='стример';

Rab1.Caption:='контроллер';
Rab2.Caption:='регистр';
Rab3.Caption:='стример';
Rab1.Checked:=true;
...
GroupBox3.Visible:=false;
  ...
  Label1.Caption:='Электронной базой ЭВМ второго поколения являются...';
Rb1.Caption:='электронные лампы';
Rb2.Caption:='полупроводники';
Rb3.Caption:='интегральные микросхемы';

Rab1.Caption:='электронные лампы';
Rab2.Caption:='полупроводники';
Rab3.Caption:='интегральные микросхемы';
Rab2.Checked:=true;
...
procedure TForm5.numb6;                                            //материнская плата
begin
 Label1.Caption:='Где находится разъем VGA';
 GroupBox1.Visible:=false;
GroupBox3.Visible:=true;

shap[1]:=2;
Img2.Picture.LoadFromFile('доп материал\backpanel.bmp');
Label2.Left:=192;

end;

procedure TForm5.numb7;                                           //материнская плата
begin
 Label1.Caption:='Где находится разъем PS/2';
 GroupBox1.Visible:=false;
GroupBox3.Visible:=true;
...
procedure TForm5.numb8;                                                 //с 8 доделать
begin
  Label1.Caption:='Где находится USB 3.0';
 GroupBox1.Visible:=false;
GroupBox3.Visible:=true;
...
begin
   Label1.Caption:='Где находится Аудио разъемы';
 GroupBox1.Visible:=false;
GroupBox3.Visible:=true;
...
begin
   Label1.Caption:='Где находится гнездо Ethernet';
 GroupBox1.Visible:=false;
GroupBox3.Visible:=true;
...
begin
    Label1.Caption:='Где находится видео разъемы';
 GroupBox1.Visible:=false;
GroupBox3.Visible:=true;
...
begin
Label1.Caption:='Где находится разъем DVI';
 GroupBox1.Visible:=false;
GroupBox3.Visible:=true;
...
if numvcl[numvopr]=1 then begin
  ...
  Shape5.Enabled:=false;
  Shape6.Enabled:=false;
  Shape7.Enabled:=false;                                       // показать выбранный вариант ответа
  end
  else begin
   GroupBox1.Enabled:=true;
   GroupBox1.Color:=clMenu;
  ...
  Shape7.Enabled:=true;
   ...
   for b:=1 to 3 do
   if numotv[numvopr,b]=1 then                               //показать выбранный вариант ответа
    if b=1 then
    Rb1.Checked:=true
    else if b=2 then
    Rb2.Checked:=true
    ...
    Button1.Visible:=false;
     ...
     for b:=1 to 7 do
   if numotv[numvopr,b]=1 then  begin                             //показать выбранный вариант ответа
    if b=1 then begin
...
procedure TForm5.sbros;                             //сброс всего !!!!!(новый пользователь)
begin
 i:=1;
kolotv:=0;
numvopr:=1;
